...
| Amount        | 是   | float  | 用户提现的货币数量，精确到 6 位小数                        |
| AddressTo     | 是   | string | 提现目标钱包                                               |
| CallBackUrl   | 否   | string | 通过这个Url 回调通知用户提现订单的状态，可不设置，默认会采用合作伙伴系统设置的回调 url |
